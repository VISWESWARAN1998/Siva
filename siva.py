...
def check_for_vulnerabilities(connection, project_id, url):
    ...
    SivaDB.update_result(
        connection=connection, project_id=project_id, phase_id="IG-PHASE-1")
    ...
    PhaseOneAnalysis(
        ...
        connection=connection)
    SivaDB.update_result(
        connection=connection, project_id=project_id, phase_id="IA-PHASE-1")
    ...
    if firewall_name == "None":
        ...
        port_scan.scan()
    SivaDB.update_result(
        connection=connection, project_id=project_id, phase_id="IG-PHASE-2")

    # =========================== INFORMATION GATHERING PHASE - 3 ============================
    print("[+] INFORMATION GATHERING PHASE-3 HAS BEEN STARTED")
    InfoGatheringPhasethree(
        project_id=project_id, url=url, thread_semaphore=thread_semaphore)
    SivaDB.update_result(
        connection=connection, project_id=project_id, phase_id="IG-PHASE-3")
    ...
    PhaseTwoAnalysis(
        ...
        connection=connection)
    SivaDB.update_result(
        connection=connection, project_id=project_id, phase_id="IA-PHASE-2")
...
def main():
    ...
    try:
        connection = pymysql.connect(
            host="localhost", user=user_name, password=password, db="siva")
